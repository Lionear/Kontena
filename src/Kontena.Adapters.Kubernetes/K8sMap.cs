...
internal static class K8sMap
{
    ...
    public static PodDisruptionBudget ToDisruptionBudget(V1PodDisruptionBudget b) => new()
    {
        ...
        Selector = b.Spec?.Selector is { } selector ? Labels(selector.MatchLabels) : null,
        SelectorExpressions =
        [
            .. (b.Spec?.Selector?.MatchExpressions ?? []).Select(e =>
                new LabelRequirement(e.Key ?? string.Empty, e.OperatorProperty ?? string.Empty, [.. e.Values ?? []])),
        ],
