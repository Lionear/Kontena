- **Kontena updates itself.** An installed copy now checks for a new version on launch, fetches it
  ...
  moment. Settings → Updates picks the channel — **Stable** for tagged releases, **Nightly** for the
  rolling build cut from `develop` — and can turn the background download off. A copy that cannot
  replace itself (unpacked from an archive, or installed by your distribution) says so plainly and
  points at the releases page instead of offering a button that would do nothing. (KON-110)
