...
path and there is nothing to conflict on.
    ...
    changelog.d/KON-107.fixed.md     ->  a bullet under "### Fixed"
    changelog.d/KON-110.added.md     ->  a bullet under "### Added"
...
def split_changelog(text: str) -> tuple[str, str, str]:
    ...
    The trailing match is `[ \t]*`, not `\s*`: `\s` includes newlines, so a greedy `\s*$` swallows the
    blank line after the heading into `before`, while the rendered body opens with one of its own —
    an extra blank line per render, since the next run then swallows two.
    """
