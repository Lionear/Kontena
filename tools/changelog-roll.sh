...
#
# Called on a Stable (v-tag) release; the build workflow commits the result back to main. A rolled
# changelog only touches a .md file, which build.yml's paths-ignore skips — so the commit-back never
# re-triggers a build.
#
# Input is env, never argv splicing:
#   VERSION    the release version, e.g. 0.1.0                 (required)
#   DATE       ISO date, e.g. 2026-07-20                       (required)
#   REPO       owner/name for the compare links (default: Lionear/Kontena)
#   CHANGELOG  path to the changelog (default: CHANGELOG.md)
set -euo pipefail

: "${VERSION:?VERSION is required (e.g. 0.1.0)}"
: "${DATE:?DATE is required (e.g. 2026-07-20)}"
REPO="${REPO:-Lionear/Kontena}"
...
# existing [Unreleased] link, if any.
