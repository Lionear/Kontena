...
# Bundled data: the same rationale as ARTWORK — no dependency closure covers a data file fetched by a
# script and vendored as a resource. Hand-kept because it changes when a Kubernetes minor is added or
# dropped, not when a package is bumped.
BUNDLED_DATA = """## Bundled data

Manifest Studio's offline schema fallback (KON-289) vendors real Kubernetes OpenAPI v3 documents,
trimmed to their `components.schemas` section, from the
[kubernetes/kubernetes](https://github.com/kubernetes/kubernetes) repository (Apache-2.0). See
`Kontena.Plugins.ManifestStudio/Resources/Schemas/README.md` for the exact source URLs, the minors
bundled and the trimming method."""
...
def main():
    ...
    out = [
        ...
        "> the bundled-artwork and bundled-data sections kept in that script — do not edit this file by",
        "> hand. Re-run it after changing dependencies or vendoring artwork/data.",
        "",
        f"## Packages ({len(rows)})",
        "",
